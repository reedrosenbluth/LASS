...
class DummyConvBlockRes(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, momentum, has_film):
        ...
        # Mock batch norms (not actually used for math, just for FiLM params)
    ...
    def forward(self, input_tensor, film_dict=None):
        # film_dict for ConvBlockRes is expected to be like: {'beta1': tensor, 'beta2': tensor}
...
class DummyEncoderBlockRes1B(nn.Module):
    ...
    def forward(self, input_tensor, film_dict=None):
        ...
        if self.has_film and film_dict is not None:
            assert 'conv_block1' in film_dict, "film_dict for EncoderBlockRes1B missing 'conv_block1'"
...
class DummyDecoderBlockRes1B(nn.Module):
    ...
    def forward(self, input_tensor, concat_tensor, film_dict=None):
        conv_block_film = None
        if self.has_film and film_dict is not None:
            assert 'conv_block2' in film_dict, "film_dict for DecoderBlockRes1B missing 'conv_block2'"
...
assert fused_pool_features.shape[0] == batch_size
assert fused_pool_features.shape[1] == enc1_out_channels * num_resolutions
assert fused_pool_features.shape[2] == time_frames // downsample_enc1[0]
assert fused_pool_features.shape[3] == num_freq_bins // downsample_enc1[1]

print(f"Fused Skip Features Shape: {fused_skip_features.shape}")
assert fused_skip_features.shape[0] == batch_size
assert fused_skip_features.shape[1] == enc1_out_channels * num_resolutions
assert fused_skip_features.shape[2] == time_frames
assert fused_skip_features.shape[3] == num_freq_bins
...
assert shared_enc2_output.shape[1] == enc2_out_channels_shared
...
assert decoder_concat_input_tensor.shape[1] == dec6_conv_block_in_channels_fused
...
assert final_decoder_block_output.shape[1] == dec6_conv_block_out_channels_final

print("--- Isolated Fusion Test Script Complete ---")
print("Review printed shapes and ensure assertions pass.")

# To run this:
# 1. Save as a Python file (e.g., test_fusion_isolated.py).
# 2. Execute `python test_fusion_isolated.py`.
# 3. Check the console output for shapes and assertion messages. 